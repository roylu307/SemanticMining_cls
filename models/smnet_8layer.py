...
class DensePoint(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, P=512, C=256, C_pts_fts=64, add_C=96, conv0=True, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, P=512, C=256, C_pts_fts=64, add_C=160, conv0=True, before_pool=True))
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, P=128, C=256, C_pts_fts=64, add_C=144, conv0=True, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, P=128, C=256, C_pts_fts=64, add_C=208, conv0=True))
        self.SA_modules.append(SphereConv(K=16, D=2, P=128, C=256, C_pts_fts=64, add_C=272, conv0=True, before_pool=True))
        # self.SA_modules.append(SphereConv(K=16, D=2, P=128, C=256, C_pts_fts=64, add_C=336, before_pool=False))
#         input_channels = 93
#         self.SA_modules.append(
#             PointnetSAModuleMSG(
#                 npoint=128,
...
class SphereConv(nn.Module):
    def __init__(self, K, D, P, C, C_pts_fts, add_C, conv0=False, before_pool=False, after_pool=False):
        ...
        self.shortcut = nn.Sequential(
                nn.Conv1d(add_C, math.floor(C/4), kernel_size=1, bias=False),
                nn.BatchNorm1d(math.floor(C/4)))

        if after_pool:
            self.bn4 = nn.BatchNorm2d(add_C, momentum=0.98)
        if before_pool:
            self.bn4 = nn.BatchNorm1d(int(C/4) + add_C, momentum=0.98)
