...
class DensePoint(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        self.SA_modules.append(SphereConv(K=32, D=4, P=128, C=256, C_pts_fts=64, add_C=96, after_pool=False))
        self.SA_modules.append(SphereConv(K=32, D=4, P=128, C=256, C_pts_fts=64, add_C=160))
        self.SA_modules.append(SphereConv(K=32, D=4, P=128, C=256, C_pts_fts=64, add_C=224, before_pool=False))
#         input_channels = 93
#         self.SA_modules.append(
#             PointnetSAModuleMSG(
#                 npoint=128,
...
class SphereConv(nn.Module):
    def __init__(self, K, D, P, C, C_pts_fts, add_C, before_pool=False, after_pool=False):
        ...
        if before_pool:
            self.bn4 = nn.BatchNorm1d(C_pts_fts + add_C, momentum=0.98)
