...
import numpy as np
from pointnet2_lib.pointnet2 import pointnet2_utils, pytorch_utils
from pointnet2_lib.pointnet2.pointnet2_modules import PointnetFPModule
...
class SM_net(nn.Module):
    ...
    def __init__(self, input_channels=0, use_xyz=True):
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0, before_pool=True))
        # self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=400, bottleneck_ratio=2, conv0=conv0))
        # self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, before_pool=True))
        # stage 1 end
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0))
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=400, bottleneck_ratio=2, conv0=conv0))
        self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, before_pool=True))
        # self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=656, bottleneck_ratio=2, conv0=conv0))
        # self.SA_modules.append(SphereConv(K=16, D=2, C=256, C_pts_fts=64, add_C=784, bottleneck_ratio=2, conv0=conv0, before_pool=True))
        ...
        self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, after_pool=False))
        self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0))
        self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=400, bottleneck_ratio=2, conv0=conv0))
        self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, before_pool=True))
...
class SphereConv(nn.Module):
    def __init__(self, K, D, P, C, C_pts_fts, add_C, bottleneck_ratio=4, conv0=False, before_pool=False, after_pool=False):
        ...
        if before_pool:
            self.bn4 = nn.BatchNorm1d(math.floor(C/bottleneck_ratio) + add_C, momentum=0.98)
...
#         print(fts.shape)
    ...
    def knn_indices_general(self, queries, points, k, sort=True, unique=False):
        ...
        _, point_indices = torch.topk(-D, k=k, sorted=True)  # (N, P, K)
        # batch_indices = torch.arange(batch_size, dtype=torch.int, device=torch.device('cuda')).view(-1, 1, 1, 1).repeat(1, point_num, k, 1)
        
        # indices = torch.cat((batch_indices, torch.unsqueeze(point_indices[:,:,:],dim=3).int()), dim=3)
