...
class DensePoint(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        # self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=656, bottleneck_ratio=2, conv0=conv0))
        # self.SA_modules.append(SphereConv(K=8, D=2, C=256, C_pts_fts=64, add_C=784, bottleneck_ratio=2, conv0=conv0, before_pool=True))
...
class SphereConv(nn.Module):
    def __init__(self, K, D, C, C_pts_fts, add_C, bottleneck_ratio=4, conv0=False, before_pool=False, after_pool=False):
        ...
        if conv0:
            self.conv0 = nn.Conv2d(in_channels=3, out_channels=C_pts_fts, kernel_size=1, bias=True)
