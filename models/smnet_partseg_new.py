...
class DensePoint(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        grouping = 'Ball_Query' # 'Ball_Query' 'KNN'
        conv0 = True
        ...
        self.SA_modules.append(SphereConv(K=32, D=2, radius=0.1, C=256, 
            C_pts_fts=64, add_C=64, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=32, D=2, radius=0.1, C=256, 
            C_pts_fts=64, add_C=192, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=True))
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.3, C=256, 
            C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, grouping=grouping, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.3, C=256, 
            C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.3, C=256, 
            C_pts_fts=64, add_C=400, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.3, C=256, 
            C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=True))
        # stage 2 end
        ...
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, grouping=grouping, after_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=400, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=656, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=784, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=912, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
            C_pts_fts=64, add_C=1040, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=True))
        # self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
        #     C_pts_fts=64, add_C=1168, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        # self.SA_modules.append(SphereConv(K=16, D=2, radius=0.5, C=256, 
        #     C_pts_fts=64, add_C=1296, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=True))
        ...
        ''' base model: K=16, D=2, P=128, C=256, C_pts_fts=64, add_C=96 '''
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=144, bottleneck_ratio=2, conv0=conv0, grouping=grouping, after_pool=False))
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=272, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
        ...
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=528, bottleneck_ratio=2, conv0=conv0, grouping=grouping))
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=656, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=784, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        self.SA_modules.append(SphereConv(K=8, D=2, radius=0.8, C=256, 
            C_pts_fts=64, add_C=912, bottleneck_ratio=2, conv0=conv0, grouping=grouping, before_pool=False))
        ...
        self.FP_modules.append(PointnetFPModule(mlp=[1168+1168, 512, 512]))
        self.FP_modules.append(PointnetFPModule(mlp=[512+656, 384, 384]))
        self.FP_modules.append(PointnetFPModule(mlp=[384+320, 256, 256]))
        self.FP_modules.append(PointnetFPModule(mlp=[256+3+19, 128, 128]))
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, cls_label):
        r"""
        ...
        for idx, module in enumerate(self.FP_modules):
            ...
            if idx==3:
                ...
                unknown_features = torch.cat([cls_label_one_hot,unknown.transpose(1,2),unknown_features],1)
...
class SphereConv(nn.Module):
    def __init__(self, K, D, radius, C, C_pts_fts, add_C, bottleneck_ratio=4, conv0=False, grouping='KNN', before_pool=False, after_pool=False):
        ...
        if grouping == 'Ball_Query':
            self.groupers = pointnet2_utils.QueryAndGroup(radius, K, use_xyz=True)

        in_channels_fts_conv = C_pts_fts + add_C
        if conv0:
            self.conv0 = nn.Sequential(
                nn.Conv2d(in_channels=3, out_channels=C_pts_fts//2, kernel_size=1, bias=False),
                nn.BatchNorm2d(C_pts_fts//2, momentum=0.98),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=C_pts_fts//2, out_channels=C_pts_fts, kernel_size=1, bias=False),
                nn.BatchNorm2d(C_pts_fts, momentum=0.98),
                nn.ReLU(inplace=True))
        else:
        ...
        if after_pool:
            self.bn4 = nn.BatchNorm1d(add_C, momentum=0.98)
        if before_pool:
    ...
    def forward(self, pts, fts_prev):
        ...
        if self.after_pool:
            fts_prev = self.bn4(fts_prev)

        if self.grouping == 'Ball_Query':
            new_features = self.groupers(pts, pts, fts_prev)  # (B, C, npoint, nsample)
            pts_grouped, fts_grouped = torch.split(new_features,[3,fts_prev.shape[1]], dim=1)
            fts = pts_grouped - pts_flipped.unsqueeze(dim=-1) # [N ,3, P, K]
            # fts, fts_grouped = self.sort_ball_query(fts, fts_grouped)

        elif self.grouping == 'KNN':
...
#         print(fts.shape)
    ...
    def sort_ball_query(self, pts, fts):
        B, C, P, K = fts.shape[0], fts.shape[1], fts.shape[2], fts.shape[3]
        d = torch.sum(pts*pts, dim=1,keepdim=True)
        _, idx = torch.sort(d, dim=-1)
        _, inv_idx = torch.sort(idx, dim=-1)
        inv_idxp = inv_idx.expand(-1,3,-1,-1)
        inv_idxf = inv_idx.expand(-1,C,-1,-1)
        sorted_pts = torch.zeros(B,3,P,K).cuda().scatter_(-1, inv_idxp, pts)
        sorted_fts = torch.zeros(B,C,P,K).cuda().scatter_(-1, inv_idxf, fts)
        return sorted_pts, sorted_fts
